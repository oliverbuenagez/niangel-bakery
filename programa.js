/* ═══════════════════════════════════════════════════
   ...
   Estructura:
    ...
    12.  Ventas multi-pan
    13.  Deudas
    14.  Historial general de ventas
    15.  Rankings y alertas
    16.  Exportar CSV
    17.  Firebase listeners
    18.  Inicialización
═══════════════════════════════════════════════════ */
...
// 3. UTILIDADES
// ══════════════════════════════════════════════════
function syncOk()   { _dot('ok',      'En la nube'); }
function syncSync() { _dot('syncing', 'Guardando...'); }
function syncErr()  { _dot('err',     'Sin conexión'); }
...
function esMismaFecha(ts, rango) {
  ...
  if (rango === 'semana') { const h7 = new Date(); h7.setDate(hoy.getDate()-7); return d >= h7; }
  if (rango === 'mes')    return d.getMonth() === hoy.getMonth() && d.getFullYear() === hoy.getFullYear();
  return true;
}

const DIAS_NOM = { lun:'Lun', mar:'Mar', mie:'Mié', jue:'Jue', vie:'Vie', sab:'Sáb', dom:'Dom' };
const DIA_IDX  = ['dom','lun','mar','mie','jue','vie','sab'];

const TIPOS = {
  tienda:'🏪 Tienda', fruver:'🥬 Fruver', restaurante:'🍽️ Restaurante',
  cafeteria:'☕ Cafetería', ambulante:'🛒 Ambulante', panaderia:'🍞 Panadería',
  vecino:'🏠 Vecino/Particular', mayorista:'📦 Mayorista', otro:'👤 Otro'
};

function emojiTipo(tipo) {
  const m = { tienda:'🏪', fruver:'🥬', restaurante:'🍽️', cafeteria:'☕',
               ambulante:'🛒', panaderia:'🍞', vecino:'🏠', mayorista:'📦', otro:'👤' };
  return m[tipo] || '👤';
}
...
// 4. MODALES
// ══════════════════════════════════════════════════
function confirmar({ icono='⚠️', titulo, desc, btnTxt='Confirmar', btnCls='btn-eliminar', accion }) {
  ...
  btn.onclick = () => { cerrarModal(); accion(); };
...
function abrirModalEditar(titulo, bodyHTML, onGuardar, btnTxt='Guardar cambios') {
...
window.irTab = function(tab, btn) {
  ...
  if (tab === 'clientes')     { dibujarClis(); dibujarHistVentas(filtroHV); }
};

// Tabs del panel derecho de clientes
window.verPanelTab = function(tab, btn) {
  ['venta','historial','ranking'].forEach(t => {
    const el = document.getElementById('panel-' + t);
    if (el) el.style.display = t === tab ? 'block' : 'none';
  });
  document.querySelectorAll('.panel-tab').forEach(b => b.classList.remove('activo'));
...
window.agregarIng = async function() {
  ...
  if (isNaN(cantidad)) { toast('Escribe la cantidad'); return; }
  if (!unidad)         { toast('Escribe la unidad'); return; }
  if (isNaN(precio))   { toast('Escribe el precio'); return; }

  const dup = ingredientes.find(i => i.nombre.toLowerCase() === nombre.toLowerCase());
  if (dup) {
    confirmar({
      icono:'⚠️', titulo:'Ya existe este ingrediente',
      desc:`"${dup.nombre}" ya está en tu bodega. ¿Quieres editarlo?`,
      btnTxt:'Editar existente', btnCls:'btn-editar',
  ...
  syncSync();
  try {
    await addDoc(collection(db,'ingredientes'), { nombre, cantidad, unidad, precio, creadoEn: serverTimestamp() });
    ['ing-nom','ing-cant','ing-uni','ing-pre'].forEach(id => document.getElementById(id).value = '');
    toast('Ingrediente guardado ✓'); syncOk();
  } catch(e) { syncErr(); toast('Error al guardar'); }
};
...
window.editarIng = function(id) {
  ...
  abrirModalEditar(`✏️ Editar: ${ing.nombre}`, body, async () => {
    ...
    if (!nombre || isNaN(cantidad) || !unidad || isNaN(precio)) { toast('Completa todos los campos'); return; }
    syncSync();
    try {
      await setDoc(doc(db,'ingredientes',id), { nombre, cantidad, unidad, precio }, { merge:true });
      cerrarModalEditar(); toast('Actualizado ✓'); syncOk();
    } catch(e) { syncErr(); toast('Error'); }
  });
};

window.eliminarIng = function(id) {
  const ing   = ingredientes.find(i => i.id === id);
  const enUso = recetas.filter(r => r.ings.some(ri => ri.ingId === id)).map(r => r.nombre);
  confirmar({
    ...
    accion: async () => {
      syncSync();
      try { await deleteDoc(doc(db,'ingredientes',id)); toast('Eliminado'); syncOk(); }
      catch(e) { syncErr(); toast('Error'); }
    }
  });
};
...
function dibujarIngs() {
  ...
  c.innerHTML = '<div class="lista">' + ingredientes.map(ing => {
...
window.crearRec = async function() {
  ...
  if (!nombre)             { toast('Escribe el nombre del pan'); return; }
  if (!rinde || rinde < 1) { toast('¿Cuántos panes rinde?'); return; }
  if (isNaN(precio) || precio <= 0) { toast('Escribe el precio de venta'); return; }
  syncSync();
  try {
    await addDoc(collection(db,'recetas'), { nombre, rinde, precio, ings:[], creadoEn: serverTimestamp() });
    ['rec-nom','rec-rinde','rec-precio'].forEach(id => document.getElementById(id).value = '');
    toast('Receta creada ✓'); syncOk();
  } catch(e) { syncErr(); toast('Error'); }
};
...
window.editarRec = function(id) {
  ...
  abrirModalEditar(`✏️ Editar: ${rec.nombre}`, body, async () => {
    ...
    if (!nombre || !rinde || rinde < 1) { toast('Completa los campos'); return; }
    syncSync();
    try {
      await setDoc(doc(db,'recetas',id), { nombre, rinde, precio: isNaN(precio)?0:precio }, { merge:true });
      cerrarModalEditar(); toast('Receta actualizada ✓'); syncOk();
    } catch(e) { syncErr(); toast('Error'); }
  });
};
...
window.eliminarRec = function(id) {
  const rec = recetas.find(r => r.id === id);
  confirmar({
    icono:'🗑️', titulo:`Eliminar "${rec.nombre}"`,
    desc:'Se eliminará la receta y sus ingredientes asociados.',
    accion: async () => {
      syncSync();
      try { await deleteDoc(doc(db,'recetas',id)); toast('Receta eliminada'); syncOk(); }
      catch(e) { syncErr(); toast('Error'); }
    }
  });
};
...
window.agregarIngRec = async function(recId) {
  ...
  if (!ingId || isNaN(cantidad) || cantidad <= 0) { toast('Selecciona ingrediente y cantidad'); return; }
  const rec = recetas.find(r => r.id === recId);
  if (rec.ings.find(i => i.ingId === ingId)) { toast('Ya está en la receta'); return; }
  syncSync();
  try {
    await setDoc(doc(db,'recetas',recId), { ings:[...rec.ings,{ingId,cantidad}] }, { merge:true });
    toast('Ingrediente agregado ✓'); syncOk();
  } catch(e) { syncErr(); toast('Error'); }
};
...
window.quitarIngRec = function(recId, ingId) {
  ...
  confirmar({
    icono:'🗑️', titulo:`Quitar "${ing?.nombre||'ingrediente'}"`,
    desc:'Se quitará de esta receta.',
    accion: async () => {
      const rec = recetas.find(r => r.id === recId);
      syncSync();
      try {
        await setDoc(doc(db,'recetas',recId), { ings: rec.ings.filter(i => i.ingId!==ingId) }, { merge:true });
        syncOk();
      } catch(e) { syncErr(); }
    }
  });
...
function dibujarRecs() {
  ...
  const optsIng = ingredientes.map(i =>
    `<option value="${i.id}">${i.nombre} (${i.unidad})</option>`).join('');

  c.innerHTML = recetas.map(rec => {
    const costo    = costoRec(rec);
    const rinde    = rec.rinde || 1;
    const precio   = rec.precio || 0;
    const ganancia = precio - (costo/rinde);
    const colorG   = ganancia >= 0 ? 'var(--verde)' : 'var(--rojo)';
    const signo    = ganancia >= 0 ? '+' : '';

    const filasIng = rec.ings.map(ri => {
      const ing = ingredientes.find(i => i.id === ri.ingId);
      if (!ing) return '';
      ...
      <div class="receta-header" id="rh-${rec.id}" onclick="toggleRec('${rec.id}')">
        <div style="flex:1;min-width:0">
          ...
          <div class="receta-meta">
            ...
            Ganancia/pan: <strong style="color:${colorG}">${signo}$${ganancia.toFixed(2)}</strong>
          </div>
        </div>
      ...
      <div class="receta-body" id="rb-${rec.id}">
        <div style="background:var(--crema-o);border-radius:9px;padding:9px 12px;
          ...
          <div style="font-size:.77rem;color:var(--texto-s)">
            Costo/pan: <strong>$${(costo/rinde).toFixed(3)}</strong>
          </div>
        </div>
...
window.cargarSelRec = function() {
  ...
  sel.innerHTML = '<option value="">Selecciona un pan</option>' +
    recetas.map(r => `<option value="${r.id}"${r.id===prev?' selected':''}>${r.nombre}</option>`).join('');
  if (prev) verIngRec();
};
...
window.verIngRec = function() {
  ...
  // Autocomplete precio desde la receta
  ...
  let html = '';
  if (panes > 0) {
    const vTxt = Number.isInteger(factor) ? `${factor}× exactas` : `${factor.toFixed(2)}× (≈${Math.ceil(factor)} prep.)`;
    html += `<div class="prod-escala-info">
      🧮 <strong>${panes} panes</strong> ÷ rinde <strong>${rinde}</strong> = repetir <strong>${vTxt}</strong>
    </div>`;
  }

  html += '<div class="lista">' + rec.ings.map(ri => {
    const ing = ingredientes.find(i => i.id === ri.ingId);
    if (!ing) return '';
    const cant = ri.cantidad * factor;
    const co   = (ing.precio/ing.cantidad) * cant;
    let extra  = '';
    if (ing.unidad==='g'||ing.unidad==='gr') extra = ` = ${(cant/1000).toFixed(3)} kg`;
    else if (ing.unidad==='ml')              extra = ` = ${(cant/1000).toFixed(3)} L`;
    return `<div class="item">
      <div class="info">
        <strong>${ing.nombre}</strong>
...
window.calcularProd = async function() {
  ...
  if (!recId)               { toast('Selecciona un tipo de pan'); return; }
  if (!panes || panes < 1)  { toast('Ingresa cuántos panes'); return; }
  if (!precio || precio<=0) { toast('Ingresa el precio de venta'); return; }
  const rec = recetas.find(r => r.id === recId);
  if (!rec.ings.length)     { toast('La receta no tiene ingredientes'); return; }
  ...
  try {
    await addDoc(collection(db,'historial'), {
      fecha:fechaTS, fechaTexto, recetaId:rec.id, recetaNombre:rec.nombre,
      panes, precio, rinde, factor, costoTotal, totalVenta, ganancia, costoPorPan, gananciaPan
    });
    syncOk();
  } catch(e) { syncErr(); }
  ...
  const vTxt = Number.isInteger(factor)?`${factor}×`:`${factor.toFixed(2)}× (≈${Math.ceil(factor)})`;

  const filasTabla = rec.ings.map(ri => {
    const ing = ingredientes.find(i => i.id === ri.ingId);
    if (!ing) return '';
    const cant = ri.cantidad * factor;
    const co   = (ing.precio/ing.cantidad)*cant;
...
window.filtrarH = function(rango, btn) {
  document.querySelectorAll('#tab-historial .filtro-btn').forEach(b => b.classList.remove('activo'));
  btn.classList.add('activo');
  filtroHist = rango;
  dibujarHist(rango);
};
...
window.eliminarHist = function(id) {
  confirmar({
    ...
    accion: async () => {
      syncSync();
      try { await deleteDoc(doc(db,'historial',id)); toast('Eliminado'); syncOk(); }
      catch(e) { syncErr(); toast('Error'); }
    }
  });
};
...
window.editarHist = function(id) {
  ...
  abrirModalEditar('✏️ Editar producción', body, async () => {
    ...
    if (!panes || panes<1 || isNaN(precio)) { toast('Datos inválidos'); return; }
    ...
    try {
      await setDoc(doc(db,'historial',id), {
        panes, precio, factor, costoTotal, totalVenta, ganancia,
        costoPorPan: costoTotal/panes, gananciaPan: precio-costoTotal/panes
      }, { merge:true });
      cerrarModalEditar(); toast('Actualizado ✓'); syncOk();
    } catch(e) { syncErr(); toast('Error'); }
  });
...
function dibujarHist(rango) {
  const filtrados = historial.filter(h => esMismaFecha(h.fecha, rango)).reverse();
...
window.filtrarStats = function(rango, btn) {
  document.querySelectorAll('#tab-estadisticas .filtro-btn').forEach(b => b.classList.remove('activo'));
  btn.classList.add('activo');
  dibujarStats(rango);
};
...
function dibujarStats(rango='todo') {
  ...
  const porPan = {};
  filtrados.forEach(h => {
    if (!porPan[h.recetaNombre]) porPan[h.recetaNombre] = { ganancia:0, veces:0, panes:0 };
    porPan[h.recetaNombre].ganancia += h.ganancia;
    porPan[h.recetaNombre].veces++;
    porPan[h.recetaNombre].panes += h.panes;
  });
  const ranking = Object.entries(porPan).sort((a,b) => b[1].ganancia-a[1].ganancia);
  const maxG = ranking[0]?.[1].ganancia || 1;

  const porTipo = {};
  clientes.forEach(cli => {
    (cli.ventas||[]).forEach(v => {
      const panes = v.panes||[{cantidad:v.cantidad,precioUnit:v.precioUnit}];
      const total = panes.reduce((t,p) => t+(p.cantidad||0)*(p.precioUnit||0), 0);
  ...
  c.innerHTML = `
    <div class="stats-grid">
      <div class="stat-card sc-dorado"><div class="stat-label">Ganancia total</div>
        <div class="stat-valor">$${tG.toFixed(2)}</div></div>
...
window.agregarCli = async function() {
  ...
  if (clientes.find(c => c.nombre.toLowerCase()===nombre.toLowerCase())) {
    toast('Ya existe un cliente con ese nombre'); return;
  }
  syncSync();
  try {
    await addDoc(collection(db,'clientes'), {
      nombre, tipo, tel, dir, ventas:[], deudas:[], pedidosReg:[],
      creadoEn: serverTimestamp()
    });
    ['cli-nom','cli-tel','cli-dir'].forEach(id => document.getElementById(id).value='');
    cerrarModalNuevoCli();
    toast('Cliente agregado ✓'); syncOk();
  } catch(e) { syncErr(); toast('Error'); }
};
...
window.editarCli = function(id) {
  ...
  if (!cli) return;
  const opTs = Object.entries(TIPOS).map(([val,label]) =>
    `<option value="${val}"${cli.tipo===val?' selected':''}>${label}</option>`).join('');
  const body = `
    <div class="campo"><label>Nombre</label>
  ...
  abrirModalEditar(`✏️ Editar: ${cli.nombre}`, body, async () => {
    ...
    syncSync();
    try {
      await setDoc(doc(db,'clientes',id), { nombre, tipo, tel, dir }, { merge:true });
      cerrarModalEditar(); toast('Cliente actualizado ✓'); syncOk();
    } catch(e) { syncErr(); toast('Error'); }
  });
};
...
window.eliminarCli = function(id) {
  const cli = clientes.find(c => c.id === id);
  confirmar({
    icono:'🗑️', titulo:`Eliminar "${cli.nombre}"`,
    desc:'Se eliminarán el cliente y todos sus datos (ventas, deudas).',
    accion: async () => {
      syncSync();
      try { await deleteDoc(doc(db,'clientes',id)); toast('Cliente eliminado'); syncOk(); }
      catch(e) { syncErr(); toast('Error'); }
    }
  });
};
...
function refrescarSelects() {
  const optsCli = '<option value="">Selecciona cliente</option>' +
    clientes.map(c => `<option value="${c.id}">${c.nombre}</option>`).join('');
  const optsRec = '<option value="">Selecciona pan</option>' +
    recetas.map(r => `<option value="${r.id}">${r.nombre}</option>`).join('');
  ['vta-cli'].forEach(id => { const el=document.getElementById(id); if(el) el.innerHTML=optsCli; });
...
function dibujarClis() {
  // Resumen
  const totalDeuda = clientes.reduce((s,c) => s+(c.deudas||[]).reduce((t,d)=>t+d.monto,0), 0);
  const hoy = new Date();
  const ventasHoy = clientes.reduce((n,c) =>
    n + (c.ventas||[]).filter(v => v.ts && esMismaFecha(v.ts,'hoy')).length, 0);
  const mesTot = clientes.reduce((s,c) =>
    s + (c.ventas||[]).filter(v => v.ts && esMismaFecha(v.ts,'mes'))
         .reduce((t,v) => {
           const p = v.panes||[{cantidad:v.cantidad,precioUnit:v.precioUnit}];
           return t + p.reduce((x,pp)=>x+(pp.cantidad||0)*(pp.precioUnit||0),0);
         }, 0), 0);
  ...
  // Alerta inactivos
  const hoy7 = new Date(); hoy7.setDate(hoy.getDate()-7);
  ...
  if (inactivos.length) {
    ...
    document.getElementById('cli-alerta-texto').innerHTML =
      ...
      inactivos.map(c => {
        const ultima = (c.ventas||[]).reduce((max,v)=>Math.max(max,v.ts||0),0);
        const dias   = ultima ? Math.floor((Date.now()-ultima)/86400000) : '?';
        return `${c.nombre} (${dias}d)`;
      }).join(' · ');
  } else {
  ...
  c.innerHTML = lista.map(cli => {
    ...
    let badges = '';
    if (esFrecuente) badges += `<span class="badge-estado badge-frecuente">⭐ Frecuente</span>`;
    if (deudaTotal > 0) badges += `<span class="badge-estado badge-debe">Debe $${deudaTotal.toFixed(2)}</span>`;
    ...
    return `
      <div class="cliente-card">
        ...
        <div class="cliente-body" id="ccb-${cli.id}">
          ...
          <div id="ct-deudas-${cli.id}" style="display:none">${_htmlTabDeudas(cli)}</div>
...
function _htmlTabVentas(cli) {
  ...
  return ventas.slice(0,10).map(v => {
    const panes  = v.panes||[{cantidad:v.cantidad,recetaNombre:v.recetaNombre,precioUnit:v.precioUnit}];
    const total  = panes.reduce((t,p)=>t+(p.cantidad||0)*(p.precioUnit||0),0);
    const resumen= panes.map(p=>`${p.cantidad} × ${p.recetaNombre}`).join(', ');
  ...
  (ventas.length>10?`<p style="font-size:.72rem;color:var(--texto-s);text-align:center;padding-top:4px">
    + ${ventas.length-10} ventas anteriores</p>`:'');
}

function _htmlTabDeudas(cli) {
  const deudas = cli.deudas||[];
  let html = deudas.length
    ? deudas.map(d=>`<div class="deuda-item">
        <div><div class="deuda-desc">${d.desc}</div><div class="deuda-fecha">${d.fecha}</div></div>
        <div style="display:flex;align-items:center;gap:7px">
          <div class="deuda-monto">$${d.monto.toFixed(2)}</div>
          <button class="btn btn-sm btn-verde" onclick="pagarDeuda('${cli.id}',${d.id})">✓ Pagó</button>
        </div>
      </div>`).join('')
    : `<p style="font-size:.82rem;color:var(--verde);padding:8px 0">✅ Sin deudas pendientes</p>`;
  html += `<div class="divider"></div>
    <p class="nota-sec">Agregar deuda manual</p>
    <div style="display:flex;gap:7px;flex-wrap:wrap">
      <input type="text" id="deu-d-${cli.id}" placeholder="Descripción" style="flex:1;min-width:110px">
      <input type="number" id="deu-m-${cli.id}" placeholder="$" step="0.01" min="0" style="max-width:75px">
      <button class="btn btn-oscuro btn-sm" style="width:auto"
...
function _htmlTabStats(cli) {
  const ventas = cli.ventas||[];
  if (!ventas.length) return `<p style="font-size:.82rem;color:var(--texto-s);padding:8px 0">Sin ventas aún.</p>`;
  const porPan = {};
  ventas.forEach(v => {
    const panes=v.panes||[{recetaNombre:v.recetaNombre,cantidad:v.cantidad}];
    panes.forEach(p => {
  ...
  return `<p class="nota-sec">Panes favoritos</p>` +
    favs.map(([nom,cant])=>`
...
window.agregarFilaVenta = function() {
  ...
  const optsRec = recetas.map(r=>`<option value="${r.id}">${r.nombre}</option>`).join('');
...
window.onSelRecVenta = function(id) {
  ...
  const rec   = recetas.find(r => r.id===recId);
  if (rec && rec.precio) {
    document.querySelector(`.vf-pre[data-fila="${id}"]`).value = rec.precio.toFixed(2);
    recalcularTotal();
  }
};
...
window.recalcularTotal = function() {
  ...
  document.getElementById('vta-total-display').textContent = `$${total.toFixed(2)}`;
};
...
window.registrarVenta = async function() {
  ...
  document.querySelectorAll('.vta-fila').forEach(fila => {
    ...
    if (!recId||!cantidad||cantidad<1||isNaN(precioUnit)||precioUnit<=0) { valido=false; return; }
    const rec = recetas.find(r => r.id===recId);
    panesData.push({ recetaId:recId, recetaNombre:rec?.nombre||'?', cantidad, precioUnit });
  });

  if (!valido||!panesData.length) {
    toast('Completa todos los panes (tipo, cantidad y precio)'); return;
  }

  const total    = panesData.reduce((t,p)=>t+p.cantidad*p.precioUnit, 0);
  const fechaTxt = fechaDeInput(fecha);
  const cli      = clientes.find(c => c.id===cliId);
  const nuevaVenta = { id:Date.now(), panes:panesData, total, pago, nota, fecha:fechaTxt, ts:Date.now() };
  const ventas = [...(cli.ventas||[]), nuevaVenta];
  let deudas   = cli.deudas||[];
  if (pago==='fiado') {
    const desc = panesData.map(p=>`${p.cantidad} ${p.recetaNombre}`).join(', ');
    deudas = [...deudas, { id:Date.now()+1, desc, monto:total, fecha:fechaTxt }];
  }
  syncSync();
  try {
    await setDoc(doc(db,'clientes',cliId), { ventas, deudas }, { merge:true });
...
window.editarVenta = function(cliId, ventaId) {
  ...
  if (!venta) return;
  const panes = venta.panes||[{cantidad:venta.cantidad,recetaNombre:venta.recetaNombre,precioUnit:venta.precioUnit}];
  const filasHTML = panes.map((p,i)=>`
  ...
  const body = `${filasHTML}
    ...
    <div class="campo" style="margin-top:11px"><label>Estado de pago</label>
      <div class="select-wrap"><select id="ev-pago">
  ...
  window.evTotal = () => {
    let t=0;
    panes.forEach((_,i)=>{
      t+=(parseFloat(document.getElementById(`ev-cant-${i}`)?.value)||0)
  ...
  abrirModalEditar('✏️ Editar venta', body, async () => {
    const nuevosPanes = panes.map((p,i)=>({
      ...p,
      cantidad:  parseInt(document.getElementById(`ev-cant-${i}`)?.value)||p.cantidad,
      precioUnit: parseFloat(document.getElementById(`ev-pre-${i}`)?.value)||p.precioUnit
    }));
    const pago  = document.getElementById('ev-pago').value;
    const total = nuevosPanes.reduce((t,p)=>t+p.cantidad*p.precioUnit,0);
    const ventas= (cli.ventas||[]).map(v=>v.id===ventaId?{...v,panes:nuevosPanes,total,pago}:v);
    syncSync();
    try {
      await setDoc(doc(db,'clientes',cliId),{ventas},{merge:true});
      cerrarModalEditar(); toast('Venta actualizada ✓'); syncOk();
    } catch(e){ syncErr(); toast('Error'); }
  });
...
window.agregarDeuda = async function(cliId) {
  ...
  const cli = clientes.find(c=>c.id===cliId);
  const nuevasDeudas = [...(cli.deudas||[]), { id:Date.now(), desc, monto, fecha:fechaHoy() }];
  syncSync();
  try {
    await setDoc(doc(db,'clientes',cliId),{deudas:nuevasDeudas},{merge:true});
    toast('Deuda registrada ✓'); syncOk();
  } catch(e){ syncErr(); toast('Error'); }
};

window.pagarDeuda = function(cliId, deuId) {
  confirmar({
    icono:'✅', titulo:'Marcar como pagada',
    desc:'¿Confirmas que este cliente pagó esta deuda?',
    btnTxt:'Sí, pagó', btnCls:'btn-verde',
    accion: async () => {
      const cli = clientes.find(c=>c.id===cliId);
...
window.filtrarHistVentas = function(rango, btn) {
  document.querySelectorAll('.filtros-compactos .filtro-btn').forEach(b=>b.classList.remove('activo'));
  btn.classList.add('activo');
  filtroHV = rango;
...
function dibujarHistVentas(rango) {
  ...
  clientes.forEach(cli => {
    (cli.ventas||[]).forEach(v => {
      ...
      if (!esMismaFecha(tsV,rango)) return;
      const panes = v.panes||[{recetaNombre:v.recetaNombre,cantidad:v.cantidad,precioUnit:v.precioUnit}];
      const total = panes.reduce((t,p)=>t+(p.cantidad||0)*(p.precioUnit||0),0);
      todasVentas.push({...v, total, clienteNombre:cli.nombre, clienteTipo:cli.tipo});
    });
  });
  ...
  if (!todasVentas.length) {
    ...
    if(lst) lst.innerHTML=`<div class="vacio"><div class="vacio-icono">🛍️</div>
                            <p>No hay ventas en este periodo.</p></div>`;
    return;
  }
  const tTotal  = todasVentas.reduce((t,v)=>t+v.total,0);
  const tPagado = todasVentas.filter(v=>v.pago==='pagado').reduce((t,v)=>t+v.total,0);
  const tFiado  = todasVentas.filter(v=>v.pago==='fiado').reduce((t,v)=>t+v.total,0);

  if(res) res.innerHTML=`
    <div class="hv-resumen-wrap">
      <div class="hv-res-card"><div class="hv-res-label">Total</div>
        <div class="hv-res-valor">$${tTotal.toFixed(2)}</div></div>
      <div class="hv-res-card"><div class="hv-res-label">Pagado</div>
        <div class="hv-res-valor" style="color:var(--verde)">$${tPagado.toFixed(2)}</div></div>
      <div class="hv-res-card"><div class="hv-res-label">Fiado</div>
        <div class="hv-res-valor" style="color:var(--rojo)">$${tFiado.toFixed(2)}</div></div>
    </div>`;

  if(lst) lst.innerHTML = todasVentas.slice(0,30).map(v => {
    const panes  = v.panes||[{recetaNombre:v.recetaNombre,cantidad:v.cantidad}];
    const resumen= panes.map(p=>`${p.cantidad} × ${p.recetaNombre}`).join(', ');
...
function dibujarRankings() {
  const rankCli = clientes.map(cli => {
  ...
  if (elRC) elRC.innerHTML = !rankCli.length
...
window.exportarCSV = function() {
  const filas = [['Fecha','Cliente','Tipo','Panes','Total','Pago','Nota']];
  clientes.forEach(cli=>{
    (cli.ventas||[]).forEach(v=>{
      const panes=v.panes||[{recetaNombre:v.recetaNombre,cantidad:v.cantidad}];
      const resumen=panes.map(p=>`${p.cantidad}x${p.recetaNombre}`).join('; ');
      filas.push([v.fecha||'',cli.nombre,TIPOS[cli.tipo]||cli.tipo,
        `"${resumen}"`,(v.total||0).toFixed(2),v.pago,v.nota||'']);
    });
  });
  ...
  a.href=url; a.download=`niangel-ventas-${hoyParaInput()}.csv`; a.click();
...
// 17. FIREBASE LISTENERS
// ══════════════════════════════════════════════════
onSnapshot(query(collection(db,'ingredientes'), orderBy('creadoEn','asc')), snap => {
  ingredientes = snap.docs.map(d=>({id:d.id,...d.data()}));
  dibujarIngs();
  if (document.getElementById('tab-recetas').classList.contains('activa')) dibujarRecs();
  syncOk();
}, ()=>syncErr());

onSnapshot(query(collection(db,'recetas'), orderBy('creadoEn','asc')), snap => {
...
onSnapshot(query(collection(db,'historial'), orderBy('fecha','asc')), snap => {
  historial = snap.docs.map(d=>({id:d.id,...d.data()}));
  if (document.getElementById('tab-historial').classList.contains('activa')) dibujarHist(filtroHist);
  syncOk();
}, ()=>syncErr());

onSnapshot(query(collection(db,'clientes'), orderBy('creadoEn','asc')), snap => {
