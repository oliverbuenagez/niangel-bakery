...
// Filtra por mes específico "YYYY-MM"
...
// Genera opciones de mes a partir de fechas reales en historial y ventas
function generarOpcionesMeses() {
  const mesesSet = new Set();
  historial.forEach(h => {
    const d = typeof h.fecha === 'string' ? new Date(h.fecha + 'T00:00:00') : new Date(h.fecha);
...
function dibujarHist(rango, mesEsp) {
  const filtrados = (rango === 'mes-especifico'
    ? historial.filter(h => esMismoMes(typeof h.fecha==='string'?new Date(h.fecha+'T00:00:00').getTime():h.fecha, mesEsp))
    : historial.filter(h => esMismaFecha(h.fecha, rango))).reverse();
...
function dibujarStats(rango='todo', mesEsp) {
  ...
  // Cobrado real a clientes en el mismo periodo
  let tCobrado = 0;
  clientes.forEach(cli => {
    (cli.ventas||[]).forEach(v => {
      const tsV = v.ts||(v.fecha ? new Date(v.fecha).getTime() : 0);
      if (rango === 'mes-especifico' ? !esMismoMes(tsV, mesEsp) : !esMismaFecha(tsV, rango)) return;
      const panes = v.panes||[{cantidad:v.cantidad,precioUnit:v.precioUnit}];
      tCobrado += panes.reduce((t,p) => t+(p.cantidad||0)*(p.precioUnit||0), 0);
    });
  });
  ...
  c.innerHTML = `
    <div class="card" style="margin-bottom:16px">
      ...
      <div class="stats-grid" style="margin-top:10px">
        ...
        <div class="stat-card" style="border-top:3px solid ${colorGR}">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card sc-verde"><div class="stat-label">Total vendido</div>
        ...
        <div class="stat-sub">según producción</div></div>
    ...
    <div class="card">
      ...
      ${(()=>{
        const rc = clientes.map(cli => {
          const tot = (cli.ventas||[]).reduce((t,v) => {
            const tsV = v.ts||(v.fecha?new Date(v.fecha).getTime():0);
            if (rango === 'mes-especifico' ? !esMismoMes(tsV,mesEsp) : !esMismaFecha(tsV,rango)) return t;
        ...
        }).filter(c=>c.tot>0).sort((a,b)=>b.tot-a.tot).slice(0,5);
        const mx = rc[0]?.tot||1;
        return rc.length ? rc.map((cl,i)=>`
          <div class="ranking-item">
            <div class="ranking-num ${i===0?'oro':''}">${i+1}</div>
            <div class="ranking-info">
          ...
          </div>`).join('') : '<p style="font-size:.82rem;color:var(--texto-s)">Sin ventas en este periodo.</p>';
      })()}
    </div>` : ''}
    ${rankTipo.length ? `
    <div class="card">
  ...
  if (inactivos.length) {
    ...
    document.getElementById('cli-alerta-texto').innerHTML =
      ...
      inactivos.map(c => {
        ...
        const dias   = ultima ? `${Math.floor((Date.now()-ultima)/86400000)}d` : 'sin compras';
...
window.registrarVenta = async function() {
  ...
  const nuevaVenta = { id:Date.now(), panes:panesData, total, nota, fecha:fechaTxt, ts:Date.now() };
...
`;
  ...
  abrirModalEditar('✏️ Editar venta', body, async () => {
    ...
    const ventas= (cli.ventas||[]).map(v=>v.id===ventaId?{...v,panes:nuevosPanes,total}:v);
    syncSync();
    try {
  ...
  clientes.forEach(cli=>{
    (cli.ventas||[]).forEach(v=>{
      ...
      filas.push([v.fecha||'',cli.nombre,TIPOS[cli.tipo]||cli.tipo,
        `"${resumen}"`,(v.total||0).toFixed(2),v.nota||'']);
    });
  });
